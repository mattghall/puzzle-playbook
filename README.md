...
- **[/connections](https://playbook.trailmatt.com/connections)** — colour and shuffle the sixteen words.
...
## Layout
    ...
    infra/            the CloudFront function that gives us clean URLs
...
That serves `dist/` on port 8080 with the same clean-URL rewriting CloudFront does,
and runs the board Lambdas in-process so `/connections/board` and `/geozee/board` work.

## Adding a game

1. `games/<name>/` with an `index.html` using **absolute** asset paths (`/<name>/main.bundle.js`).
   Relative paths break at `/<name>` because the browser resolves them against `/`.
