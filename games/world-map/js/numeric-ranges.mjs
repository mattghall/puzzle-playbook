...
export const NUMERIC_METRICS = [
    ...
    { key: "capital_population", path: "capital.cities[].population", unit: "people", sourceIds: ["capital_population_over_x", "captial_population_under_x"] },
...
export function validateNumericValue(metric, value) {
    ...
    if (metric.key === "capital_population") {
        if (!Array.isArray(value) || !value.length || value.some(population => population !== null && (!Number.isSafeInteger(population) || population < 0))) {
...
export function matchNumericValue(value, side, threshold) {
    const values = Array.isArray(value) ? value : [value];
    if (values.some(actual => actual !== null && (side === "lower" ? actual > threshold : side === "upper" ? actual < threshold : actual === threshold))) return true;
...
function getStops(metric, originals, countries) {
    ...
    const known = countries.flatMap(country => country.numericValues[metric.key]).filter(value => value !== null);
        ...
        for (const [side, direction, operation] of [["lower", "Over", ">"], ["upper", "Under", "<"], ["exact", "Exactly", "="]]) {
            ...
            const generated = values.map((value, variantId) => {
                ...
                for (const [code, country] of countries) {
                    ...
                    const result = matchNumericValue(actual, side, value);
                    if (result === null || metric.key === "coastline_length" && country.landlocked === null) unknown.add(code);
                    else if (result) matches.add(code);
                }
                const unavailableReason = unknown.size === countries.length ? originals.find(choice => choice.unavailableReason)?.unavailableReason : null;
                const citySource = metric.key === "capital_population" ? snapshot.source?.capitals : null;
                const source = {
                    name: "Derived " + operation + " condition from GeoGrid " + (citySource ? "common/cities.json" : "combined.json"),
                    url: citySource?.url || snapshot.source?.url || "https://cdn-assets.teuteuf.fr/data/geogrid/combined.json",
